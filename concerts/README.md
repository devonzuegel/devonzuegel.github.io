...
The static app is published at `https://devonzuegel.com/concerts/` through GitHub Pages. It keeps a browser-local notebook; cross-device sync requires a running API with durable storage. Cloud storage is not connected yet. Local preview notebooks in `.data/` are separate from the published site and are not uploaded by deployment.
